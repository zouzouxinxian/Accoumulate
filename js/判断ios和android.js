        ...
        return new Promise(function(resolve, reject) {
            jsonp(baseUrl + 'user/info/jsonp_get_current', {name:'callback'}, function (error, response) {
                if (error) {
                    ...
                    // smzdm_id为0表示未登录
                    if (typeof response.smzdm_id ==='undefined' || response.smzdm_id === 0) {
                        resolve(false);
                    }else{
                        resolve(true);
                    }
    ...
    nativeLogin(){
        ...
        if (iosOrAdrStr === 'android') {
            if (typeof call_client_login === 'function') {
                call_client_login();
            } else if (typeof window.call_client_login === 'function') {
                window.call_client_login();
            } else if (typeof window.smzdmwebaction.call_client_login === 'function') {
                window.smzdmwebaction.call_client_login();
            }
